...
function Balance() {
    ...
    const populatebalance =(result:any)=>{
        console.log('result', result)
        result && setBalance(result.bal)
        result && setaddress(result.address)
        !result && setBalance([])
    }

    useEffect(() => {
          !callController('', 'account/balance', 'get', {}, [], populatebalance, '', 'Error getting balance', 'silent') && populatebalance('')
    }, [])
    useEffect(() => {
...
<div className="w-full bg-white border mt-5 border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
    ...
    <ul className="hidden text-sm font-medium text-center text-gray-500 divide-x divide-gray-200 rounded-lg sm:flex dark:divide-gray-600 dark:text-gray-400" id="fullWidthTab" data-tabs-toggle="#fullWidthTabContent" role="tablist">
        <li className="w-full">
            <button id="stats-tab" data-tabs-target="#stats" type="button" role="tab" aria-controls="stats" aria-selected="true" className="inline-block w-full p-4 rounded-tl-lg bg-gray-50 hover:bg-gray-100 focus:outline-none dark:bg-gray-700 dark:hover:bg-gray-600">Balance</button>
        </li>
        {/* <li className="w-full">
    ...
    <div id="fullWidthTabContent" className="border-t border-gray-200 dark:border-gray-600">
        {/* <div className="p-4 bg-white rounded-lg md:p-8 dark:bg-gray-800" id="stats" role="tabpanel" aria-labelledby="stats-tab"> */}
            <dl className="flex justify-around max-w-screen-xl grid-cols-3 gap-8 p-4 mx-auto text-gray-900 sm:grid-cols-3 xl:grid-cols-6 dark:text-white sm:p-8">
                
                {balance.length > 0 ? balance.map((data, index)=>(
                    <div key={index} className="flex flex-col items-center justify-center">
                        <dt className="mb-2 text-3xl font-extrabold">{numberformat(data.balance)}</dt>
                        <dd className="text-gray-500 dark:text-gray-400">{data.asset_type == 'native' ? 'XLM' : data.asset_code}</dd>
                    </div>
                )) 
            :
                <p>This account has not been activated. fund account to activate <button
                    onClick={()=>{
                        const action =(result:any)=>{
                            console.log('result', result)
                            callModal('Account Balance Loading....')
                        }
                        const log:any = toast.loading('validating')
                        callController(log, 'account/fundaccount', 'get', {}, [], action, 'account activated successfully', 'activation failed')
                    }}
                ><span className='font-bold text-[green] p-2 bg-white rounded-md m-1'>Activate</span></button></p>
                }
            </dl>
                {address && <button onClick={() => {
                    navigator.clipboard.writeText(address);
                    callModal('copied');
                }} className='text-white p-2 mt-[-30px] bg-transparent border-none italic'>{address}</button>}
        {/* </div> */}
